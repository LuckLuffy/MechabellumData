...
|------|------|------|
| 攻击力 | — | 单管单发伤害 |
| 对单输出 | 攻击力 × 武器数 | 多武器单位：暴雨×4、鬼鳐×2、先知×2、恶灵×4、霸主×4、泰山×4、战争工厂×2 |
| 爆发峰值 | 对单输出 × 数量 | 雷霆 ×3、深渊 ×10（特殊倍率）|
| 对单DPS | 对单输出 ÷ 攻击间隔 | 深渊例外：爆发峰值 ÷ 间隔 |
| 总DPS | 对单DPS × 数量 | 全队持续输出 |
| 总血量 | 单体血量 × 数量 | 全队血量 |
| 输出性价比 | 对单DPS ÷ 造价 | 单位造价输出效率 |
| 血量性价比 | 单体血量 ÷ 造价 | 单位造价血量效率 |

> **深渊**：爆发峰值 ×10（十束射线），对单DPS 用爆发峰值÷间隔（持续输出含倍率）
> **雷霆**：爆发峰值 ×3（三管齐射），对单DPS 用对单输出÷间隔（持续DPS用单管）
