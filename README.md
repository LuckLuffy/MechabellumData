...
1. 下载 index.html
2. 双击用浏览器打开
3. 无需服务器、无需注册、无需联网 —— 离线查看全部 37 个单位数据
...
| **前端展示** | 37 单位数据表（攻击力/对单输出/爆发峰值/对单DPS/总DPS/性价比等），排序/筛选/详情弹窗，公式/日志选项卡，悬停高亮 |
...
| 攻击力 | — | 单管单发伤害（表内原始列） |
| 弹药量 | — | 一次攻击的弹药数（表内原始列） |
| 对单输出 | 攻击力 × 弹药量 | 一次攻击打满单个目标的总伤害 |
| 爆发峰值 | 对单输出 × 数量 × 倍率 | 全队单次齐射上限；仅雷霆倍率 ×3 |
| 对单DPS | 对单输出 ÷ 攻击间隔 | 持续单体输出 |
| 总DPS | 对单DPS × 数量 | 全队持续输出 |
| 总血量 | 单体血量 × 数量 | 全队血量 |
| 输出性价比 | 总DPS ÷ 造价 | 单位造价输出效率 |
| 血量性价比 | 总血量 ÷ 造价 | 单位造价血量效率 |

> **多弹药单位**（弹药量 > 1）：深渊×10、泰山×4、霸主×4、恶灵×4、暴雨×4、战争工厂×2、先知×2、鬼鳐×2
> **雷霆**：一道攻击放 3 道闪电、各打各的目标 —— 打单个目标只吃得到一道（对单输出不含 ×3），
> 但全队齐射上限要吃满 3 道，所以只有爆发峰值 ×3。
> 对单输出／爆发峰值／对单DPS 三列已从数据表下线，全部由「攻击力 + 弹药量」推算。
