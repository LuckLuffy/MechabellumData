...
/* ===== 主内容（用满横向宽度，减少横向滚动） ===== */
main{width:100%;padding:16px 16px 60px}
...
thead th{position:sticky;top:0;z-index:5;background:var(--surface2);padding:11px 8px;text-align:left;border-bottom:2px solid var(--border);cursor:pointer;user-select:none;white-space:nowrap;font:600 13px var(--sans);color:var(--dim);letter-spacing:.5px}
thead th:hover{color:var(--text)}
/* 冻结第一列（兵种名）：横向滚动时保持可见 */
thead th:first-child{position:sticky;left:0;z-index:6;border-right:1px solid var(--border-bright);min-width:140px}
tbody td:first-child{position:sticky;left:0;z-index:4;background:var(--surface);border-right:1px solid var(--border-bright)}
tbody tr:hover td:first-child{background:rgba(255,180,84,.14)}
thead th.sorted{color:var(--accent)}
thead th.sorted::after{content:' ▲';font-size:9px;color:var(--accent)}
thead th.sorted.desc::after{content:' ▼'}
tbody td{padding:10px 8px;border-bottom:1px solid var(--border);white-space:nowrap}
tbody tr{transition:background .08s}
/* 悬停变色：琥珀色高亮，显眼易辨 */
tbody tr:hover td{background:rgba(255,180,84,.13)}
tbody tr:hover td{color:#fff}
tbody tr:hover .name-cell{color:var(--accent)}
...
/* ===== 公式页 ===== */
.formula-card{max-width:900px;margin:24px auto;background:var(--surface);border:1px solid var(--border);border-radius:8px;padding:28px}
.formula-card h3{font:700 20px var(--display);margin-bottom:10px}
.formula-sub{color:var(--dim);margin-bottom:16px;font-size:13px}
.formula-table{width:100%;border-collapse:separate;border-spacing:0;font-size:14px}
.formula-table th{background:var(--surface2);padding:10px 12px;text-align:left;border-bottom:2px solid var(--border);font:600 13px var(--sans);color:var(--dim)}
.formula-table td{padding:10px 12px;border-bottom:1px solid var(--border)}
.formula-table td:first-child{font-weight:600;color:var(--accent);white-space:nowrap}
.formula-table td:nth-child(2){font-family:var(--mono);white-space:nowrap}
.formula-list{list-style:none;padding:0;margin-top:8px}
  ...
  <div id="tab-formula" style="display:none">
    <div class="formula-card">
      ...
      <table class="formula-table">
        ...
        <tr><td>爆发峰值</td><td>= 对单输出 × 数量 × 倍率</td><td>全队单次齐射上限；仅雷霆倍率 ×3</td></tr>
      ...
      <ul class="formula-list">
        ...
        <li><b>雷霆</b>：一道攻击放 3 道闪电、各打各的目标 —— 打单个目标只吃得到一道（对单输出不含 ×3），但全队齐射上限要吃满 3 道，故只有爆发峰值 ×3</li>
